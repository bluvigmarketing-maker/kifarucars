...
export default async function AdminOverviewPage() {
  ...
  return (
    <div>
      <h1 className="font-display text-2xl text-charcoal-950">Overview</h1>
      <p className="mt-1 text-sm text-charcoal-500">
        Manage the fleet, enquiries and reviews shown on kifarucarhire.com.
      </p>
      ...
      <div className="mt-8 grid grid-cols-1 gap-5 sm:grid-cols-3">
        {cards.map(({ label, value, hint, href, Icon }) => (
          <Link
            key={label}
            href={href}
            className="rounded-2xl border border-charcoal-100 bg-white p-6 transition-shadow hover:shadow-md"
          >
            <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-burgundy-50 text-burgundy-600">
              <Icon size={18} />
            </div>
            <p className="mt-4 text-sm text-charcoal-500">{label}</p>
            <p className="mt-1 font-display text-3xl text-charcoal-950">{value}</p>
