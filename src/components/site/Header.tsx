    ...
    <header className="sticky top-0 z-50 border-b border-charcoal-100 bg-white/95 backdrop-blur">
      <Container className="flex h-18 items-center justify-between py-3">
        <Link href="/" className="flex items-baseline gap-2">
          <span className="font-display text-2xl font-semibold text-charcoal-950">
            Kifaru
          </span>
        ...
        <nav className="hidden items-center gap-7 lg:flex">
          {NAV_LINKS.map((link) => (
            <a
              key={link.label}
              href={link.href}
              className="text-sm font-medium text-charcoal-700 transition-colors hover:text-burgundy-700"
            >
              {link.label}
            </a>
          ))}
        ...
        <button
          ...
          className="inline-flex items-center justify-center rounded-full p-2 text-charcoal-900 lg:hidden"
          onClick={() => setOpen((v) => !v)}
        >
          ...
          <Container className="flex flex-col gap-1 py-3">
            {NAV_LINKS.map((link) => (
              <a
                ...
                className="rounded-lg px-2 py-2.5 text-sm font-medium text-charcoal-800 hover:bg-charcoal-50"
              >
                {link.label}
              </a>
            ))}
