...
export function ReviewsTable({ reviews }: { reviews: Review[] }) {
  ...
  return (
    <div className="space-y-3">
      {reviews.map((review) => (
        <div key={review.id} className="rounded-2xl border border-charcoal-100 bg-white p-5 dark:border-charcoal-800 dark:bg-charcoal-900">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-1.5 font-medium text-charcoal-950 dark:text-white">{review.author_name}</p>
              <p className="text-xs text-charcoal-400 dark:text-charcoal-500">{review.days_ago_label}</p>
            </div>
          ...
          <p className="mt-3 text-sm text-charcoal-700 dark:text-charcoal-300">{review.body}</p>
        </div>
      ))}
